...
**Direct HTTP write (regional path):** `FleetRegistryInternalController` exposes `POST /internal/fleet/server-directory` — an internal endpoint (M2M API key required) for regional `server-service` instances to write directly to the EU user-service. This is necessary because regional Redpanda is independent — a `ServerCreatedEvent` published to the HIL Redpanda does not reach EU Redpanda where `ServerDirectoryConsumer` is listening.
...
**The solution** is the dual-write described in section 3: regional `server-service` calls `POST /internal/fleet/server-directory` on the EU user-service directly, in addition to publishing to its local Kafka. The HTTP call is synchronous-on-create — if it fails, the server create fails (fail-closed: a server whose fleet entry cannot be recorded should not be created). Status updates and deletes also flow through both paths.
